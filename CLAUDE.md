...
**[1층] KB 전세대출 4개 상품요건 × [2층] HUG/HF/SGI 보증요건** 에 병렬 대조해서
...
src/lib/crawlers/                 KB·HUG·HF·SGI 검증형 크롤러 + HTTP·provider(폴백·캐시)
...
src/rules/*.json                  상품4·보증공통·기관3 규칙팩 폴백
...
- `PathResult` = { path, guaranteeProvider, blockedAt, results, officialReviewCount }

## API 계약 (프론트-백 이 형태 유지)

- `POST /api/check` body: DiagnosisCase → `{ pathResults, ruleVersion, ruleSource, caseId? }` (caseId는 로그인 사용자만)
...
크롤링 시도 → 성공 시 1시간 캐시 사용 → 실패 시 `src/rules/*.json` 전체 폴백.
...
- [완료] F05/F06 상품 4개·보증 공통·HUG/HF/SGI 기관별 규칙팩, 병렬 판정, 공식 페이지 7개 검증형 실크롤러.
